                    ...
                    (while (> 6 triflag)
                        ...
                        (if (equal? hexColor colorCheck2)
                            ...
                            (begin
                                ...
                                (gimp-image-select-contiguous-color theImage CHANNEL-OP-ADD baseLayer selectx selecty)
                                
                                ; Use a completely random color if numColors=0
                                (if (= inNumColors 0)
                                    (set! inColor (list (rand 255) (rand 255) (rand 255)))
                                    (begin
                                ...
                                (if (= (car (gimp-selection-is-empty theImage)) FALSE)
                                    (begin
                                        ...
                                        (gimp-edit-bucket-fill baseLayer BUCKET-FILL-BG LAYER-MODE-NORMAL 100 0 FALSE 0 0)
                ...
                (begin
                    ...
                    (while (< (- ycenter (* s3 hexSize)) inHeight)
                        (begin
                            ...
                            (gimp-image-select-contiguous-color theImage CHANNEL-OP-ADD baseLayer 0 selecty)
                            ;(gimp-fuzzy-select baseLayer (- inWidth 1) selecty 0 CHANNEL-OP-ADD 0 0 0 0)
                            (gimp-image-select-contiguous-color theImage CHANNEL-OP-ADD baseLayer (- inWidth 1) selecty)
                            (set! colorCheck (car (gimp-image-pick-color theImage baseLayer xcenter selecty 0 0 0)))
                            (gimp-context-set-foreground colorCheck)
                            (gimp-edit-bucket-fill baseLayer BUCKET-FILL-FG LAYER-MODE-NORMAL 100 0 FALSE 0 0)
                    ...
                    (if (= Shape 0)
                        (while (< xcenter inWidth)
                            ...
                            (gimp-image-select-contiguous-color theImage CHANNEL-OP-ADD baseLayer selectx (- inHeight 1))
...
(script-fu-register
    "script-fu-HexorTri"
    "<Toolbox>/Script-Fu/Render/Pattern/Hexagon or Triangle Grid..."  ; edit this line to change where it shows up
    "Creates a grid of regular hexagons.  They can be colored in with random colors, tiled, or both. \nfile: JMS-Map_HexorTri_1.scm"
    "James Sambrook"
    "James Sambrook"
    "May 2011"
    ""
    ...
    SF-TOGGLE       "Tileable Array?"   FALSE
