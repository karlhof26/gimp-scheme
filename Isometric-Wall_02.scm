        ...
        (if (= leftwall TRUE)
            (begin
                (set! layer-iso-wall1 (car (gimp-layer-new img width new-height
                            type-d (string-append img-name "_ISO_L") 100 LAYER-MODE-NORMAL)))
                    ...
                    (while (<= num-of-col width)
                        ...
                        (gimp-edit-copy drw)
                        (let (
                                (floating-sel (car (gimp-edit-paste layer-iso-wall1 FALSE))))
        ...
        (if (= rightwall TRUE)
            (begin
                (set! layer-iso-wall2 (car (gimp-layer-new img width new-height
                                                type-d (string-append img-name "_ISO_R") 100 LAYER-MODE-NORMAL)))
                (gimp-image-insert-layer img layer-iso-wall2 0 0)
                (while (<= num-of-col width)
                    
                    ;(gimp-rect-select img num-of-col 0 2 height CHANNEL-OP-REPLACE FALSE 0) 
                    (gimp-image-select-rectangle img CHANNEL-OP-REPLACE num-of-col 0 2 height) 
                    (gimp-edit-copy drw)
