...
;;;;begin the script	
    ...
    (gimp-image-add-layer image bkg-layer (+ (car (gimp-image-get-layer-position image layer)) 1)) ;under
    ...
    (if (= type 2)
        (begin
            ...
            (gimp-image-add-layer image tint-layer (car (gimp-image-get-layer-position image bkg-layer))) ;over
    ...
    (if (> type 2)
        (begin
            ...
            (gimp-image-add-layer image grad-layer (car (gimp-image-get-layer-position image bkg-layer))) ;over
    ...
    (if (= trans FALSE)
        (begin
            ...
            (gimp-image-add-layer image bkg-col-layer (+ (car (gimp-image-get-layer-position image bkg-layer)) 1)) ;under
...
(script-fu-register "script-fu-impact"              
  "Impact..."
  "creates 'Blast' like lines around a layer with alpha or selectin"
  "Graechan"
...
(define (script-fu-impact-logo
  ...
  (let* (
    ...
    (gimp-image-add-layer image bkg-layer (+ (car (gimp-image-get-layer-position image text-layer)) 1)) ;under
...
(script-fu-register "script-fu-impact-logo"
  ...
  SF-FONT       "Font"               "Franklin Gothic Heavy"
