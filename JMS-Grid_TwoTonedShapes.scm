    ...
    (let* (
        ...
        (cond
            ((= constColor 0)	; Constant Color in Rows
                ...
                (set! baseLayer (car (gimp-layer-new theImage inHeight inWidth RGBA-IMAGE "Constant Rows" 100 LAYER-MODE-NORMAL)))
                (set! innerLayer (car (gimp-layer-new theImage inHeight inWidth RGBA-IMAGE "Inner Shapes" 100 LAYER-MODE-NORMAL)))
                (set! outerLayer (car (gimp-layer-new theImage inHeight inWidth RGBA-IMAGE "Outer Shapes" 100 LAYER-MODE-NORMAL)))
            )
            ((= constColor 1)	; Constant Color in Rows
                (set! theImage (car (gimp-image-new inWidth inHeight RGB)))
                (set! baseLayer (car (gimp-layer-new theImage inWidth inHeight RGBA-IMAGE "Constant Columns" 100 LAYER-MODE-NORMAL)))
                (set! innerLayer (car (gimp-layer-new theImage inWidth inHeight RGBA-IMAGE "Inner Shapes" 100 LAYER-MODE-NORMAL)))
                (set! outerLayer (car (gimp-layer-new theImage inWidth inHeight RGBA-IMAGE "Outer Shapes" 100 LAYER-MODE-NORMAL)))
            )
        )
        ...
        (if (= layered TRUE)
            (begin
                (gimp-image-insert-layer theImage outerLayer 0 0)
                (if (> shrinkDiff 0)
                    (gimp-image-insert-layer theImage innerLayer 0 0)
                )
            )
        )
        ...
        (while (< xFlag xCirc)
            ...
            (while (< yFlag yCirc)
                    ...
                    ((= shape 0)
                        (if (= constColor 0)
                            (begin
                                ...
                                (gimp-context-set-antialias aliasing)
                                (gimp-image-select-ellipse theImage CHANNEL-OP-REPLACE yStart xStart bigcircDiam bigcircDiam)
                            )
                            (begin
                                (gimp-context-set-antialias aliasing)
                                (gimp-image-select-ellipse theImage CHANNEL-OP-REPLACE xStart yStart bigcircDiam bigcircDiam)
                            )
                        )
                    )
                    ((= shape 1)
                        (if (= constColor 0)
                            (begin
                                ...
                                (gimp-image-select-rectangle theImage CHANNEL-OP-REPLACE yStart xStart bigcircDiam bigcircDiam)
                            )
                            (begin
                                ;(gimp-rect-select theImage xStart yStart bigcircDiam bigcircDiam CHANNEL-OP-REPLACE FALSE 0)
                                (gimp-image-select-rectangle theImage CHANNEL-OP-REPLACE xStart yStart bigcircDiam bigcircDiam)
                            )
                        )
                    )
                    ((= shape 2)
                ...
                (if (= layered TRUE)
                    (gimp-edit-bucket-fill outerLayer BUCKET-FILL-FG LAYER-MODE-NORMAL 100 255 0 0 0)
                ...
                (if (> shrinkDiff 0)
                    (begin
                        ...
                        (if (= layered TRUE)
                            ...
                            (gimp-edit-bucket-fill baseLayer BUCKET-FILL-BG LAYER-MODE-NORMAL 100 255 0 0 0)
                        )
                    )
                )
...
(script-fu-register
    ...
    "Grid - Two-toned shapes..."
    "Creates a grid of X by Y two-toned shapes, either in rectangular or hexagonal packing. \nfile:JMS-Grid_TwoTonedShapes.scm"
    "James Sambrook"
    "22 April 2011"
    "James Sambrook.  King George, VA, USA"
    ""
    SF-OPTION       "Shape"                      '("Circle"
                                                    "Square"
                                                    "Hexagon"
                                                    "Octagon")
    SF-OPTION       "Grid Type"                  '("Rectangular"
                                                    "Hexagonal")
    SF-ADJUSTMENT   "Outer Shape Diameter"       '(40 10 200 1 5 0 0)
    SF-ADJUSTMENT   "Inner Shape Diameter"       '(30 10 200 1 5 0 0)
    SF-ADJUSTMENT   "Shapes in X Direction"      '(10 1 100 1 5 0 0)
    SF-ADJUSTMENT   "Shapes in Y Direction"      '(11 1 100 1 5 0 0)
    SF-ADJUSTMENT   "Outer Border Around Shapes" '(12 1 100 1 5 0 0)
    SF-ADJUSTMENT   "Gap Between the Shapes"     '(13 1 100 1 5 0 0)
    SF-COLOR        "Outer shape color #1"       '(0 0 0)
    SF-COLOR        "Inner shape color #1"       '(127 127 127)
    SF-COLOR        "Outer shape color #2"       '(127 127 127)
    SF-COLOR        "Inner shape color #2"       '(0 0 0)
    SF-OPTION       "Constant colors along rows or columns?"     '("Rows"
                                                                    "Columns")
    SF-TOGGLE       "Completely random colors?"   FALSE
    SF-COLOR        "Background Color"           '(255 255 255)
    SF-TOGGLE       "Anti-aliasing?"             FALSE
    SF-TOGGLE       "Inner and outer shapes on different layers?"    TRUE
)
