...
;
;  23.11.2020 - fixed for Gimp-2.10.22
;
; --------------------------------------------------------------------
;
...
(define (script-fu-eg-isonoisereduction InImage InLayer InType InOpacity InRadius InFlatten)
    ...
    (let* (
        ...
        (let* (
                (NoiseLayerMask (car (gimp-layer-create-mask NoiseLayer ADD-MASK-SELECTION)))
              )
            ...
            (cond
                ...
                ((= InType 0)
                    (begin
                        (gimp-image-set-component-active InImage CHANNEL-RED TRUE)
                        (gimp-image-set-component-active InImage CHANNEL-GREEN FALSE)
                        (gimp-image-set-component-active InImage CHANNEL-BLUE FALSE)
                        (plug-in-sel-gauss TRUE InImage NoiseLayer RadiusRB DeltaRB)
                        ;
                        (gimp-image-set-component-active InImage CHANNEL-RED FALSE)
                        (gimp-image-set-component-active InImage CHANNEL-GREEN TRUE)
                        (plug-in-sel-gauss TRUE InImage NoiseLayer InRadius InDelta)
                        ;
                        (gimp-image-set-component-active InImage CHANNEL-GREEN FALSE)
                        (gimp-image-set-component-active InImage CHANNEL-BLUE TRUE)
                        (plug-in-sel-gauss TRUE InImage NoiseLayer RadiusRB DeltaRB)
                        ;
                        (gimp-image-set-component-active InImage CHANNEL-RED TRUE)
                        (gimp-image-set-component-active InImage CHANNEL-GREEN TRUE)
                ...
                ;
                ((= InType 2)
                    (plug-in-despeckle TRUE InImage NoiseLayer InRadius 1 7 248)
                )
            )
...
(script-fu-register "script-fu-eg-isonoisereduction"
    "<Toolbox>/Script-Fu/Photo/Enhancement/Eg ISO Noise Reduction"
    "Reduce sensor noise at high ISO values. \nfile:egger-iso-noise-reduction.scm"
