...
(define (script-fu-make-rect width height color name)
  (let* (   (old-fg-color (car (gimp-context-get-foreground)))
    ...
    (gimp-image-insert-layer img labellayer 0 -1)
    (gimp-selection-all img)
    (gimp-edit-clear labellayer)

    (gimp-context-set-foreground color)
    (gimp-edit-bucket-fill labellayer
                           BUCKET-FILL-FG LAYER-MODE-NORMAL 100 0 FALSE 0 0)
  ...
  (let* (
     ...
     (if (not (null? templ))
         (let* (
           ...
           (gimp-image-insert-layer newimg baselayer 0 -1)
...
(let* ((labelnames nil)
  ...
  (script-fu-register "script-fu-rect-label"
                      "<Toolbox>/Script-Fu3/Labels/Rect label..."
 ...
 (script-fu-register "script-fu-rect-label-page"
                     "<Toolbox>/Script-Fu3/Labels/Make label page..."
                     ...
                     SF-OPTION     "Template Name" labelnames
;                     SF-OPTION     "Paper Size"    pnames
                     SF-ADJUSTMENT  "From"          '( 1 1 99 1 10 0 1)
...
(script-fu-register "script-fu-CD-label"
            "<Toolbox>/Script-Fu3/Labels/CD label..."
            "CD label shape  \nfile:labels.scm"
            "Akkana Peck"
            "Akkana Peck"
            "December 2002"
...
(script-fu-register "script-fu-CD-mask"
            "<Image>/Script-Fu3/Labels/CD mask..."
