                        ...
                        (if (> opac 0)
                            (begin
                                (set! layer-interpol-opt (car (gimp-layer-new img width height
                                    type-d (string-append img-name "_interpolation") opac LAYER-MODE-NORMAL)))
                                (gimp-image-insert-layer img layer-interpol-opt 0 0)
                                (let (
                                        (floating-sel (car (gimp-edit-paste layer-interpol-opt FALSE)))
                                      )
        ...
        (if (= iso-floor TRUE)
                ...
                (begin
                        ...
                        (set! layer-iso-floor-tmp (car (gimp-layer-new img new-width new-height
                                type-d (string-append img-name "_ISO_tmp") 100 LAYER-MODE-NORMAL)))
                        (gimp-image-insert-layer img layer-iso-floor-tmp 0 0)
                        ...
                        (while (< num-of-row height)
                                ...
                                (gimp-edit-copy drw)
                                (let (
                                        (floating-sel (car (gimp-edit-paste layer-iso-floor-tmp FALSE)))
                                     )
                        ...
                        (set! layer-iso-floor (car (gimp-layer-new img new-width new-height
                                type-d (string-append img-name "_ISO") 100 LAYER-MODE-NORMAL)))
                        (gimp-image-insert-layer img layer-iso-floor 0 0)
...
(script-fu-register
    ...
    "<Toolbox>/Script-Fu2/2D Isometric-Fu/01 - Isometric Floor"
    "It turns a flat image into isometric image. This helps to draw floors. It can also scale image with 
    few options (ratio, a second layer with interpolation and opacity) \nfile:Isometric-Floor_v1.0_02.scm"
    "Loïc Guyader (froGgy)"
    "Copyright"
    "02/2011"
    "" ;; types d'images supportes par le script
        ...
        SF-TOGGLE       "Convert to RGB color mode"	    FALSE
        SF-TOGGLE       "Scale image:"                  FALSE
        SF-TOGGLE       "Keep aspect ratio! (the minimum ratio is the reference)" FALSE
        SF-ADJUSTMENT   "Width"                        '(256 4 2048 1 8 0 0)
        SF-ADJUSTMENT   "Height"                       '(256 4 2048 1 8 0 0)
        SF-OPTION       "Interpolation"                '("Cubic" "Linear" "Sinc (Lanczos3)")
        SF-ADJUSTMENT   "Opacity"                      '(50 0 100 1 10 1 0)
        SF-TOGGLE       "Isometric floor"              TRUE
) ;; fin du register
