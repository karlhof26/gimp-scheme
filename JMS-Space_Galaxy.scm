...
(define (add-under-layer-galaxy img newlayer oldlayer)
    (gimp-image-insert-layer img newlayer 0 (+ (get-layer-pos-galaxy img oldlayer) 1))
)
...
(define (script-fu-galaxy
  ...
  (let* (
            ...
            (baseLayer (car (gimp-layer-new theImage imageSize imageSize RGBA-IMAGE "Galaxy" 100 LAYER-MODE-SCREEN)))
            (cloudLayer (car (gimp-layer-new theImage imageSize imageSize RGBA-IMAGE "Plasma" 100 LAYER-MODE-OVERLAY)))
            (starLayer (car (gimp-layer-new theImage imageSize imageSize RGBA-IMAGE "Stars" 100 LAYER-MODE-NORMAL)))
            (scratchLayer 0)
            (novaLayer 0)
            (area 0)
            (blackLayer 0)
        ...
        (gimp-image-select-rectangle theImage CHANNEL-OP-ADD 0 diameter imageSize outerEdge)
        ...
        (gimp-edit-bucket-fill baseLayer BUCKET-FILL-FG LAYER-MODE-NORMAL 100 255 FALSE 0 0)
        ...
        (gimp-image-insert-layer theImage scratchLayer 0 0)
        ...
        (gimp-image-insert-layer theImage blackLayer 0 0)
        (gimp-layer-set-mode blackLayer LAYER-MODE-NORMAL)
        (gimp-context-set-background '(0 0 0))
        (gimp-drawable-fill blackLayer FILL-BACKGROUND)
        ...
        (gimp-image-insert-layer theImage starLayer 0 0)
        ...
        (while (< flag (/ NumStars area))
            (begin
                ...
                (set! starAngle (/ (* *pi* (rand 3590.0)) 1800.0))
                ...
                (gimp-image-select-rectangle theImage CHANNEL-OP-REPLACE Star_X Star_Y starSize starSize)
                
                (gimp-context-set-foreground (list (rand 255) (rand 255) (rand 255)))
                (gimp-edit-bucket-fill starLayer BUCKET-FILL-FG LAYER-MODE-NORMAL 100 255 FALSE 0 0)
                (set! flag (+ 1 flag))
                
                (set! progressInd (/ flag (/ NumStars area)))
                (gimp-progress-update progressInd)
            )
        )
        (gimp-selection-none theImage)
        
        (gimp-image-insert-layer theImage cloudLayer 0 0)
        
        (plug-in-plasma RUN-NONINTERACTIVE theImage cloudLayer (rand 1000000000) 7)
        (gimp-drawable-desaturate cloudLayer DESATURATE-LUMINANCE)
        ...
        (gimp-image-insert-layer theImage novaLayer 0 0)
        (gimp-item-set-name novaLayer "Nova")
        (gimp-layer-set-mode novaLayer LAYER-MODE-NORMAL)
        (gimp-context-set-background '(0 0 0))
        (gimp-drawable-fill novaLayer FILL-BACKGROUND)
        ...
        (gimp-image-insert-layer theImage scratchLayer 0 0)
        (gimp-drawable-fill scratchLayer FILL-BACKGROUND)
        ...
        ;(script-fu-layerfx-outer-glow theImage testLayer cloudColor 50 0 0 0 0 5.0 FALSE FALSE)
        ...
        (plug-in-hsv-noise RUN-NONINTERACTIVE theImage scratchLayer 1 179 254 252)
        ...
        (set! frameName (string-append "Galaxy " (number->string seed) "-" (number->string frameNum)))
        (gimp-item-set-name baseLayer frameName)
...
(script-fu-register
    "script-fu-galaxy"
    "Galaxy..."
    "Creates a picture of a spiral galaxy. \n:file:JMS-Space_Galaxy.scm"
    "James Sambrook"
    "23 February 2011"
    ""
    ""
    SF-ADJUSTMENT  "Number of arms"     '(8 4 36 1 5 0 0)
    SF-ADJUSTMENT  "Hole Size"          '(25 0 100 1 10 0 0)
    SF-ADJUSTMENT  "Inner Radius"       '(100 10 1000 1 10 0 0)
    SF-ADJUSTMENT  "Arm Size"           '(500 10 1000 1 50 0 0)
    SF-ADJUSTMENT  "Angle first arm starts at"      '(0 0 360 1 30 0 0)
    SF-ADJUSTMENT  "Arm Curvature"      '(6 0 10 0.1 1 1 0)
    SF-ADJUSTMENT  "Number of Stars"    '(200 0 3000 100 500 0 0)
    SF-ADJUSTMENT  "Maximum star size (in pixels)"  '(3 1 10 1 1 0 0)
    SF-ADJUSTMENT  "Horizontal skew"    '(1 0.1 5 0.1 1 1 0)
    SF-ADJUSTMENT  "Vertical skew"      '(1 0.1 5 0.1 1 1 0)
    SF-ADJUSTMENT  "Skew Angle"         '(60 -89 89 1 10 0 0)
    SF-COLOR       "Galaxy color"       '(100 149 237)
    SF-VALUE       "Random seed"        "123456"

)

(script-fu-menu-register "script-fu-galaxy"
	"<Image>/Script-Fu2/Render/SambrookJM/")