...
(define (script-fu-layerfx-wall-bevel-emboss img
  ...
  (let* (
    ...
    (gimp-drawable-levels highlightmask HISTOGRAM-VALUE  0.5 1.0 TRUE 1.0 0.0 1.0 TRUE)
    ;(gimp-levels shadowmask 0 0 127 1.0 255 0)
    (gimp-drawable-levels shadowmask HISTOGRAM-VALUE 0.0 0.5 TRUE 1.0 1.0 0.0 TRUE)
...
(define (script-fu-brickwall
    ...
    (let* (
            ...
            (mortarLayer (car (gimp-layer-new theImage inWidth inHeight RGBA-IMAGE "Mortar" 100 LAYER-MODE-NORMAL)))
            (noiseLayer (car (gimp-layer-new theImage inWidth inHeight RGBA-IMAGE "Noise" 100 LAYER-MODE-NORMAL)))
                    ...
                    (begin
                        ...
                        (if (= (car (gimp-selection-is-empty theImage)) FALSE)
                            (begin
                                ...
                                (gimp-edit-bucket-fill brickLayer BUCKET-FILL-BG LAYER-MODE-NORMAL 100 255 0 0 0)
                            )
                        )
        ...
        (gc) ; an array was used so cleanup
    )
)
...
(script-fu-register
    "script-fu-brickwall"
    "Brick Wall"
    "Making a wall, one brick at a time.  Thanks to mahvin, zarnad, and froGgy factory for some additions to the script. \nfile:JMS-Brickwall.scm"
    "James Sambrook"
