  ...
  (let* ((img (car (gimp-item-get-image adraw)))
    ...
    (if(= edge TRUE)
       (begin
         ...
         (gimp-drawable-invert edge-layer TRUE)
    ...
    (gimp-displays-flush)
    (gimp-context-pop)
    ; we used an arroay so do a garbagee collection
    (gc)
  )
)
...
(script-fu-menu-register "elsamuko-movie-300" "<Toolbox>/Script-Fu/Artistic")
