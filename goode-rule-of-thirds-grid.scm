...
(define (script-fu-thirds-rule-grid image drawable color line-width image-derived?)
    ...
    (let* (
        ...
        (gimp-image-select-rectangle image CHANNEL-OP-ADD w1/3 0    line-width height)
        ;(gimp-rect-select image                          0    h1/3 width      line-width CHANNEL-OP-ADD FALSE 0)
        (gimp-image-select-rectangle image CHANNEL-OP-ADD 0    h1/3 width      line-width)
        
        ;(gimp-rect-select image                          w2/3 0    line-width height     CHANNEL-OP-ADD FALSE 0)
        (gimp-image-select-rectangle image CHANNEL-OP-ADD w2/3 0    line-width height)
        ;(gimp-rect-select image                          0    h2/3 width      line-width CHANNEL-OP-ADD FALSE 0)
        (gimp-image-select-rectangle image CHANNEL-OP-ADD 0    h2/3 width      line-width)
...
(script-fu-register "script-fu-thirds-rule-grid"
                    "Rule Of Thirds Grid Layer"
                    "Generate a new layer with a Rule Of Thirds grid. \nUsing selections not guides \nfile:goode-rule-of-thirds-grid.scm"
                    "Saul Goode"
                    "Saul Goode"
                    "2007/11/1"
                    "RGB* GRAY*"
