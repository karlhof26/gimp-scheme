...
(define (my-layer-stuff     myImage
    ...
    (let* (
        ...
        (gimp-image-insert-layer myImage firstTemp 0 0)
        
        ;Desaturate the layer
        (gimp-drawable-desaturate firstTemp DESATURATE-LIGHTNESS)
        
        ;Copy and add the dodge me layer as the thin layer
        (set! thinTemp (car (gimp-layer-copy firstTemp 1)))
        (gimp-image-insert-layer myImage thinTemp 0 0)
        ...
        (if (= modeOp 1)
            ...
            (gimp-layer-set-mode thinTemp LAYER-MODE-LIGHTEN-ONLY) ; was 10
            ;Else Change the mode of the thin layer to darken
            (gimp-layer-set-mode thinTemp LAYER-MODE-DARKEN-ONLY) ; was 9
        )
        ...
        (gimp-image-insert-layer myImage thickTemp 0 1)
        ...
        (gimp-layer-set-mode merged LAYER-MODE-DIFFERENCE) ; was 6
        ...
        (if (= modeOp 1)
            (begin
                ...
                (gimp-layer-set-mode merged LAYER-MODE-DODGE) ; was 16
            )
            (begin
                ...
                ;Change the mode of the dodge copy layer to dodge mode
                (gimp-layer-set-mode merged LAYER-MODE-BURN) ; was 17
                ;Invert layer
                (gimp-drawable-invert merged FALSE) ; might be true
            )
        )
...
(define (script-fu-fake-hdr-effect inImage inDrawable Opa-num)
    ...
    (let* (
        ...
        (gimp-image-insert-layer inImage theNewlayer1 0 0)
        (gimp-layer-set-opacity theNewlayer1 75)
        
        (gimp-image-insert-layer inImage theNewlayer2 0 0)
        ...
        (set! layerRGB (car (gimp-drawable-levels theNewlayer2 HISTOGRAM-VALUE 0.392 1.0 TRUE 1.0 0.0 1.0 TRUE)))
        ...
        (gimp-image-insert-layer inImage theNewlayer3 0 0)
        ...
        (set! subdra (gimp-drawable-levels theNewlayer3 HISTOGRAM-VALUE 0.1 0.9 TRUE 1.0 0.0 1.0 TRUE))
...
(script-fu-register "script-fu-fake-hdr-effect"
    "<Toolbox>/Script-Fu/Enhance/Fake HDR Effect..."
