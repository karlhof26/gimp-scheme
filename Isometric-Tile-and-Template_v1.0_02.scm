...
(define (script-fu-00-isometric-tile-and-template
  ...
  (let* (
            ...
            (tile (car (gimp-layer-new img tilewidth tileheight
                                RGBA-IMAGE "isometric tile" 100 LAYER-MODE-NORMAL)))
            (template (car (gimp-layer-new img width height
                                RGBA-IMAGE "isometric template" 100 LAYER-MODE-NORMAL)))
            (path 0)
        ...
        (gimp-image-insert-layer img tile 0 0)
        (gimp-context-set-background bg-color)
        (if (= transp FALSE)
            (gimp-drawable-fill tile FILL-BACKGROUND)
        )
    ...
    (if (= notemplate FALSE)
        (let* (
            ...
            (gimp-image-insert-layer img template 0 0)
            (set! template (car (gimp-image-get-active-drawable img)))
            (gimp-selection-all img)
            (gimp-context-set-pattern name)
            (gimp-drawable-fill template FILL-PATTERN)
...
(script-fu-register
    ...
    SF-STRING     "Pattern name"            "ISO-tile"
    SF-STRING     "File name"               "ISO-tile"
    SF-TOGGLE     "No template! Just create the tile in my patterns!"       FALSE
...
(script-fu-menu-register "script-fu-00-isometric-tile-and-template"
                         "<Toolbox/Script-Fu2/2D Isometric-Fu")
